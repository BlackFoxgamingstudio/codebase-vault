# Engineering Roadmap & Implementation Status — Sovereign Codebase Vault

**Package ID**: `PKG-000`  
**Domain**: ** Static Analysis & Codebase Graphs  
**Microservice Port**: `http://127.0.0.1:8766`  
...
| **Networking & API** | REST Microservice (`PORT 8766`) | ✅ Complete | OpenAPI 3.1 spec, Swagger UI at `/docs` |
...
- **Role**: Handles all astparser operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `astparser`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 2: `CallGraphBuilder`
- **Role**: Handles all callgraph operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `callgraphbuilder`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 3: `DependencyAnalyzer`
- **Role**: Handles all dependencyanalyzer operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `dependencyanalyzer`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 4: `DeadCodeDetector`
- **Role**: Handles all deadcodedetector operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `deadcodedetector`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.

### Component 5: `SemanticSearchEngine`
- **Role**: Handles all semanticsearch operations. Exposes async methods callable from the core dispatcher.
- **Current Status**: ⏳ Queued for v1.1.0 Implementation
- **Integration**: Exposed via `POST /api/v1/execute` with action `semanticsearchengine`
- **Verification Strategy**: Dedicated `unittest.TestCase` validating deterministic output and idempotency.
...
1. Create discrete module file: `src/codebase_vault_<component>.py`
