# Developer Guide: Sovereign Codebase Vault
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/codebase-vault.git
cd codebase-vault
...
```bash
export SBB_VAULT_INDEX_PATH=your_value_here
export SBB_EMBEDDING_MODEL=your_value_here
export SBB_MAX_FILE_SIZE_MB=your_value_here
export SBB_EXCLUDE_PATTERNS=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-codebase-vault)
...
```bash
# Build
docker build -t sovereign-codebase-vault:dev .

# Run with env file
docker run --env-file .env -p 8766:8766 sovereign-codebase-vault:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-codebase-vault
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8766`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-codebase-vault",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8766` in use | `lsof -i :8766` | Kill PID or change `SERVICE_PORT` |
